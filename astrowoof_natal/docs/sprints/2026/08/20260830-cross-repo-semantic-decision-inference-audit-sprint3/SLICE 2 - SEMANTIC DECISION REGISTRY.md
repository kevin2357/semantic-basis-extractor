...
| R-03 / W-03, P-06 | Availability preflight | Availability v0.1 → exact result ID → exact sealed result/receipt/snapshot/journal validation → native run join | Permission only to classify and ingest that exact result; availability itself grants none | No invocation-returned result exists; named cycle preflight | `discover_available_result` then `read_exact_sealed`; outcome tested with `is_terminal_native_outcome` | Normal absence continues the ordinary selector; malformed/conflicting refuses; unknown result schema refuses | Exact result ID; discovery cannot replace invocation result |
...
| R-28 / P-06 | No invocation result → named availability preflight → exact read/validation | Discover and ingest only exact available result | runtime preflight | Normal absence continues the ordinary selector; malformed/contradictory refuses | Prove no “latest” substitution after a command returned an ID |
