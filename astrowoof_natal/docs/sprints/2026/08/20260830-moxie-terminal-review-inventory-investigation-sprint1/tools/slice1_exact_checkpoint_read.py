"""Perform the single authorized Moxie checkpoint HEAD and streaming GET."""
...
def main() -> None:
    ...
    if key != "v1/checkpoint/" + object_uuid.replace("-", "").lower():
        raise ValueError("Object key does not join the frozen UUID")
    ...
    if manifest.get("approved_operations") != {
        ...
        raise ValueError("Access manifest operations are not closed")
    ...
    head = client.head_object(Bucket=values["ASTROWOOF_R2_BUCKET"], Key=key)
    observed_etag = str(head.get("VersionId") or head.get("ETag") or "")
    expected_etag = text(manifest.get("expected_etag"), "expected ETag")
    if observed_etag != expected_etag:
        raise ValueError("R2 ETag/version does not match the frozen manifest")
    if head.get("ContentLength") != manifest.get("expected_archive_bytes"):
        raise ValueError("R2 content length does not match the frozen manifest")

    args.output_archive.parent.mkdir(parents=True, exist_ok=True)
    digest = hashlib.sha256()
    observed_bytes = 0
    # Authorized remote operation 2/2: one exact streaming GET.
    response = client.get_object(Bucket=values["ASTROWOOF_R2_BUCKET"], Key=key)
    body = response["Body"]
    try:
        with args.output_archive.open("xb") as handle:
            while chunk := body.read(64 * 1024):
                ...
                if observed_bytes > manifest["expected_archive_bytes"]:
                    raise ValueError("R2 object exceeded its frozen byte size")
                digest.update(chunk)
                handle.write(chunk)
    finally:
        body.close()
