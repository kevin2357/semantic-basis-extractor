...
- The ledger identifies a claim-specific center of gravity, recognizable
  behavior, likely handler misreading, grounded surprise, memorable image, and
  narrative angle.
- The narrative angle was selected for this claim rather than rotated by index
  or claim type.
...
- Any concrete scene is supported by the evidence rather than selected from a
  rotating scene pool.
- A scene was omitted when it added no interpretive value.
...
- verify that changing introductory clauses did not conceal an unchanged body
  template.
- normalize away dog names, claim quotations, scenes, astrology sources, and
  priority labels mentally; reject prose whose remaining architecture is
  repeated.
...
- perform the shuffled-card test and a semantic-story audit; lexical uniqueness
  and zero linter warnings are not sufficient.
- reject repeated handler lessons, emotional arcs, behavioral sequences,
  tensions, practical conclusions, and subject-specific refrains.
...
- A deck with all or nearly all filter arrays empty requires manual review and
  concrete justification; emptiness must not be used to skip classification.
