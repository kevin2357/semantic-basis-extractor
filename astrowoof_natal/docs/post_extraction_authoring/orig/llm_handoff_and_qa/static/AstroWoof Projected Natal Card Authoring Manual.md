...
## 37. Still to be discussed

Editing passes still need a dedicated process specification. That discussion should define the number and order of semantic, character-voice, humor, audience, grammar, repetition, and final acceptance passes, along with the exact conditions that send a card back to claim review. It should also define how edits are logged and whether revisions occur directly in the authoritative JSON or through generated editor-friendly views.

The process to use when file-generation or editing tooling is unavailable also remains to be defined. At minimum, the workflow should distinguish temporary chat-only drafting from authoritative file modification, preserve source attachments, avoid pretending that a file was changed when it was not, and provide a safe resumption point. It should also specify when a script-based merge is acceptable and what validation must be performed afterward.

Additional future work includes schema normalization, deterministic generation tests, card-ID migration rules, controlled vocabularies, JSON Schema validation, application rendering contracts, release packaging, and versioned example fixtures. Those topics should be discussed after the current natal-card production process is stable enough that the schema reflects real use rather than speculation. This manual intentionally ends by recording those open items instead of inventing final answers before the workflow has been exercised further.
