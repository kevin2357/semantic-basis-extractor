# AstroWoof Natal Authoring 0.4.6 Compatibility

Status: release-candidate contract

| Component | Required version | Published wheel SHA-256 |
|---|---:|---|
| Astrology Graph Foundry | 0.8.1 | `860c48793318c82c986b32664cd0f3fe97c4b1e02fb1e489561bc395c8b5a3ed` |
| Semantic Projection Core | 0.11.0 | `82290df44fe5697e87df2e27eb0aa4bab3b7954c66ce988efda0962964e1366d` |
| SBE | 0.4.6 candidate | final hash pending authorized versioned rebuild |

Python 3.11 or newer is required. Qualification covered Linux CPython 3.11 and
Windows CPython 3.12.

Exact Natal accepts the existing static projected semantic graph contract. Bounded
Natal accepts the separate `projected_bounded_semantic_graph.v1` four-context
family and never reconstructs midpoint/exact placements, strengths, or confidence.

Legacy bounded authoring-run v1 workspaces remain inspectable historical evidence
but cannot be reinterpreted as the v2 six-pass topology. Resume fails closed with
`legacy_bounded_topology_unsupported`.
