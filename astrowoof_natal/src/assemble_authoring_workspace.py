...
def assemble(
    ...
    *,
    ...
    if not cards_root.is_dir():
        raise ValueError(f"Missing cards directory: {cards_root}")
    for story_dir in sorted(path for path in cards_root.iterdir() if path.is_dir()):
        match = STORY_DIR_PATTERN.match(story_dir.name)
        if not match:
            raise ValueError(f"Unexpected story directory name: {story_dir.name}")
        priority_id = int(match.group(1))
        claim_id = match.group(2)
        source_card = cards_by_priority.get(priority_id)
        if source_card is None:
            raise ValueError(f"Unknown priority ID in workspace: {priority_id}")
        if claim_id != source_card["claim_id"]:
            raise ValueError(
                f"Story {priority_id}: directory claim ID {claim_id} does not "
                f"match packet {source_card['claim_id']}"
            )
