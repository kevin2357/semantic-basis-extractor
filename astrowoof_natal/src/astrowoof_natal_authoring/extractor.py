...
def render_summary_gold_reference(repo_root: Path) -> str:
    reference = json.loads(
        read_resource_text(
            "references/natal.kevin.summary-gold-source.cards.json"
        )
    )
    lens_names = (
        "Who He Is",
...
def write_opaque_authoring_checker(target: Path, repo_root: Path) -> None:
    ...
    package_modules = {
        "assemble_authoring_workspace": "astrowoof_natal_authoring.assembly",
        "lint_astrowoof_editorial": "astrowoof_natal_authoring.editorial_lint",
        "lint_authoring_pass": "astrowoof_natal_authoring.pass_acceptance",
    }
    sources = {
        name: module_source_path(package_modules[name]).read_text(encoding="utf-8")
...
def copy_static_assets(
    ...
    rigorous_sources = {
        "Semantic Basis Extractor Pipeline and Scoring Metrics.md":
            "authoring/Semantic Basis Extractor Pipeline and Scoring Metrics.md",
        "Proposed LLM Handoff Prompt.md":
            "authoring/Proposed LLM Handoff Prompt.md",
        "LLM Editing Permissions and QA Checklist.md":
            "authoring/LLM Editing Permissions and QA Checklist.md",
        "LLM Card-by-Card Authoring Execution Protocol.md":
            "authoring/LLM Card-by-Card Authoring Execution Protocol.md",
        "AstroWoof Independent Card Writing Brief.md":
            "authoring/AstroWoof Independent Card Writing Brief.md",
        "AstroWoof Projected Natal Card Authoring Manual.md":
            "authoring/AstroWoof Projected Natal Card Authoring Manual.md",
        "AstroWoof Authoring Packet Schema.json":
            "schemas/AstroWoof Authoring Packet Schema.json",
        "AstroWoof Bre Editorial Gold Reference.json":
            "references/AstroWoof Bre Editorial Gold Reference.json",
    }
    compact_sources = {
        "AstroWoof Compact Single-Subject Authoring Brief.md":
            "authoring/AstroWoof Compact Single-Subject Authoring Brief.md",
        "Compact LLM Handoff Prompt.md":
            "authoring/Compact LLM Handoff Prompt.md",
        "AstroWoof Authoring Packet Schema.json":
            "schemas/AstroWoof Authoring Packet Schema.json",
        "AstroWoof Bre Editorial Gold Reference.json":
            "references/AstroWoof Bre Editorial Gold Reference.json",
    ...
    for name, source in sources.items():
        copy_resource(source, static / name)
...
def main() -> None:
    ...
    input_group.add_argument(
        ...
        help=(
            ...
            "or one immediate child directory per subject. Required for the "
            "installed CLI."
        ),
    )
    input_group.add_argument(
    ...
    if input_package is None:
        parser.error("--input-package is required")
    ...
    if args.handoff_profile == "authoring-workspace":
        ...
        copy_resource(
            "authoring/Multi-Subject LLM Handoff README.md",
            args.bundle_dir / "README.md",
        )
    copy_module_source(
        "astrowoof_natal_authoring.validation",
        args.bundle_dir / "validate_astrowoof_editorial.py",
    )
    copy_module_source(
        "astrowoof_natal_authoring.editorial_lint",
        args.bundle_dir / "lint_astrowoof_editorial.py",
    )
    if args.handoff_profile == "authoring-workspace":
        copy_module_source(
            "astrowoof_natal_authoring.assembly",
            args.bundle_dir / "assemble_authoring_workspace.py",
        )
    run_records = []
    failed = False
